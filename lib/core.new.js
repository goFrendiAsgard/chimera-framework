...
const KEY_ORDER = ['id', 'if', 'vars', 'ins', 'out', 'command', 'mode', 'chains', 'while']
const KEY_SYNONYM = {
    'process': 'command',
    'input': 'ins',
    'inputs': 'ins',
...
function splitCommandStringByShortArrow (commandString) {
    ...
    } else if (commandParts.length == 2 && commandParts[0].match(MATCH_INSIDE_PARANTHESES_PATTERN)) {
        // pattern: (ins) -> command
        return {
            'ins': commandParts[0],
            'command': commandParts[1],
...
function splitCommandString (commandString) {
    let obj = splitCommandStringByLongArrow(commandString)
    if (util.isNullOrUndefined(obj)) {
        obj = splitCommandStringByShortArrow(commandString)
    }
    // if ins surounded by (), unwrap it
    if (obj.command.match(MATCH_INSIDE_PARANTHESES_PATTERN)) {
        return {
            'ins': unwrap(obj.ins),
            'command': obj.command,
            'out': obj.out
...
function defineTrueScriptForNormalJs (chain) {
    ...
    chain.command = '(...args) => {let _callback = args.pop(); let _fn = '+command+'; let _result = _fn(...args); callback(null, _result);}'
    return chain
}
...
function defineTrueScriptForPromiseJs (chain) {
    let command = unwrap(chain.command)
    chain.command = '(...args) => {let _callback = args.pop(); let _promise = '+command+'; _promise.then((value)=>{callback(null, value);}, (error)=>{callback(error, null)});}'
    return chain
}

function defineTrueScriptForCmd (chain) {
    let command = chain.command
    chain.command = '(...args) => {CMD_EXECUTOR('+util.quote(command)+', ...args)}'
    return chain
}
...
if (require.main === module) {
    ...
    // runChain(trueChain, [[4,5,1,2,3], 'yha'])
}
