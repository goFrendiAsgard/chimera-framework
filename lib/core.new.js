...
function ExecCmd (...args) {
  ...
  let option = args.shift(args)
  for (let value of args) {
    command += ' ' + stringify(value)
  }
  cmd.get(command, option, callback)
}

function callbackifyNormalJs (command) {
  return (...args) => {
...
function callbackifyPromiseJs (command) {
  return (...args) => {
    let context = Object.assign({}, VARIABLES)
    let callback = args.pop()
    let promise = saveEval(command, context)
    promise.then((result) => { callback(null, result) })
  }
}

function callbackifyCmd (command) {
  return (...args) => {
    let context = Object.assign({}, VARIABLES)
    let callback = args.pop()
    for (let value of args) {
...
function defineTrueScriptForNormalJs (chain) {
  ...
  chain.command = 'callbackifyNormalJs(' + util.quote(command) + ')'
  return chain
}

function defineTrueScriptForCallbackJs (chain) {
  chain.command = unwrap(chain.command)
  return chain
}

function defineTrueScriptForPromiseJs (chain) {
  let command = unwrap(chain.command)
  chain.command = 'callbackifyPromiseJs(' + util.quote(command) + ')'
  return chain
}

function defineTrueScriptForCmd (chain) {
  let command = chain.command
  chain.command = 'callbackifyCmd(' + util.quote(command) + ')'
  return chain
}
...
function tidyUpChain (chain) {
  ...
  for (let key of KEY_ORDER) {
    if ((key in chain) && !(key in exceptionKeys)) {
