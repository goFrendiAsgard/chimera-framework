...
let getFilteredObject = corePreprocessor.getFilteredObject
...
function getPublishedState () {
    let filteredState = getFilteredObject(VARIABLES, ['$'])
    return getInspectedObject(filteredState)
}
...
function createSingleChainCallback(chain, next, finalCallback){
    return (error, result) => {
        ...
        try {
            // TODO: eval is evil, not really. But we need to define the middle keys if they are undefined, so we can't simply use eval
            eval('VARIABLES.'+chain.out+' = result')
            logMessage('(chain #' + chain.id + ') Set '+chain.out+' into: ' + getInspectedObject(evaluateStatement(chain.out)), VERY_VERBOSE)
...
function createSingleChainAction (chain, finalCallback) {
    return (next) => {
        try {
            ...
            logMessage('(chain #' + chain.id + ') input values: ' + getInspectedObject(inputs), VERBOSE)
            if(VARIABLES._error) {
...
function runRootChain (chain, insValue = [], vars = {}, finalCallback = null) {
    ...
    logMessage('CHAIN SEMANTIC:\n'+getInspectedObject(chain), VERBOSE)
