...
function executeChain (chain, ins, vars, callback) {
  ...
  function setVariable (name, val) {
    ...
    if (!('__proxy' in VARIABLES)) {
      VARIABLES.__proxy = createObjectProxy(VARIABLES)
    }
    let scriptVal = JSON.stringify(getNormalizedValue(val))
    let script = '(() => {__proxy.' + name + ' = ' + scriptVal + '; delete this.__proxy})()'
