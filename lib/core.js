        ...
        if(checkCondition(chain.if)){
            ...
            if('chains' in chain && util.isArray(chain.chains)){
                ...
                actions.push((next)=>{
                    ...
                    try{
                        log('(chain '+chain.id+') Run ' + JSON.stringify(chain.command) + ' with arguments: '+ JSON.stringify(inputs), SOMEHOW_VERBOSE)
...
function load(...args){
    // get the module
    console.error(typeof(args))
    console.error(args)
    let moduleName = args.shift()
    console.error(typeof(moduleName), moduleName)
