...
function execute(chainConfigs, argv, presets, finalCallback, chainOptions){
    ...
    function callFinalCallback(error, result, errorMessage){
        ...
        finalCallback(output, success, errorMessage)
    }
    ...
    function getSingleArrowFunctionChainRunner(chainCommand, chainOut, parameters, callback){
        ...
        try{
            ...
            if(getVar('_error')){
                ...
                callFinalCallback(false, '', errorMessage)
            }
            else{
                // continue the chain
                callback()
            ...
            console.error('[ERROR] SCRIPT : ' + jsScript)
            callFinalCallback(false, '', e.stack)
        }
    }
        ...
        try{
            cmd.get(cmdCommand, {'cwd': chainOptions.cwd}, function(error, stdout, stderr){
                ...
                if(!error){
                    ...
                    callFinalCallback(false, '', error.stack)
                }
            })
        }
        catch(error){
...
function executeChain(chain, argv, presets, finalCallback){
    ...
    fs.readFile(chain, function(err, data){
        ...
        // get chainConfig
        let chainConfig = getChainConfig(chainString)
        // ensure we going back to this directory
        let wrappedCallback  = wrapCallback(finalCallback)
        execute(chainConfig, argv, presets, wrappedCallback, chainOptions)
    })
}
