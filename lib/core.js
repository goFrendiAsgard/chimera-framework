...
function getChainFromShortArrow(str){
    ...
    if(chainShard.length >= 3){
        ...
        return {'ins':[], 'command':chainShard[0], 'out':'_ans', 'original_command':chainShard[0]}
    }
    return null
}
    ...
    if(util.isString(command)){
        ...
        if(matchCallback || matchPromise || matchAnonymousFunction){
            try{
                ...
                if(matchCallback){
                    // match callback
                    log('Creating callbacked function for '+command+' (OPT_CALLBACK)', VERBOSE)
                    return callbackify(eval(command), OPT_CALLBACK)
                }
                else if(matchPromise){
                    // match promise
                    log('Creating callbacked function for '+command+' (OPT_PROMISE)', VERBOSE)
                    return callbackify(eval(command), OPT_PROMISE)
                }
                else{
                    // match anonymous function
                    log('Creating callbacked function for '+command+' (OPT_NORMAL)', VERBOSE)
            ...
            // command runner
            log('Creating cmd executor for '+command, VERBOSE)
            return (...args) => {
...
function patchAndTrimObjChain(objChain){
    let isSingle = 'command' in objChain && util.isFunction(objChain.command)
    // objChain
    objChain = util.patchObject(DEFAULT_CHAIN, objChain)
    let allowedProperties = ['id', 'if', 'while', 'else', 'error', 'error_message']
    if(isSingle){
        allowedProperties = allowedProperties.concat(['original_command', 'command', 'ins', 'out'])
    }
    else{
        ...
        if(checkCondition(chain.if)){
            ...
            if('chains' in chain && util.isArray(chain.chains)){
                ...
                actions.push((next)=>{
                    ...
                    try{
                        log('(chain '+chain.id+') Run ' + JSON.stringify(chain.original_command) + ' with arguments: '+ JSON.stringify(inputs), SOMEHOW_VERBOSE)
    ...
    util.readJsonOrYaml(chain, function(error, chainObj){
        if(!error){
            ...
            util.parseJsonOrYaml(chain, function(error, chainObj){
                if(!error){
                    vars._description = 'SCRIPT: '+chain
                    runRootChain(chainObj, ins, vars, finalCallback)
                }
                else{
                    console.error(error)
...
function load(module, fn=null){
    let chain_cwd = getVar('_chain_cwd')
    if(chain_cwd != null){
        module = chain_cwd +'/'+ module
    }
