...
let preprocessor, chimlParser, safeEval, stringify, async, fs, util, path, $
...
function executeChain (chain, ins, vars, callback) {
  ...
  fs.stat(chainPath, (error, chainStat) => {
    if (error) {
      // no, it is not a file. it must be a CHIML script
      vars._description = 'script: ' + util.getSlicedString(chainScript, 50)
    ...
    // is chain has already been compiled and still relevant?
    if (chainPath in CHIML_COMPILATION_MEMO && chainStat.mtime < CHIML_COMPILATION_MEMO[chainPath].time) {
      return executeChainScript(CHIML_COMPILATION_MEMO[chainPath].script, ins, vars, callback)
    }
    // chain is CHIML file
  ...
  function compileAndExecuteFile (chainPath, compiledChainPath) {
    fs.readFile(chainPath, (error, chainScript) => {
      ...
      return chimlParser.parseChiml(chainScript, (error, chainObj) => {
        ...
        preprocessor = requireOnce('./core-preprocessor.js')
        ...
        util.writeJsonFile(compiledChainPath, chainObj, (error) => {
          ...
          let compiledChainScript = stringify(chainObj)
          CHIML_COMPILATION_MEMO[chainPath] = {time: new Date(), script: compiledChainScript}
          return executeChainScript(compiledChainScript, ins, vars, callback)
        })
      })
    })
  ...
  function evaluateStatement (statement, log = false) {
    ...
    try {
      ...
      try {
        ...
        // script
        safeEval = requireOnce('safe-eval')
  ...
  function runRootChain (chain, insValue, vars, finalCallback) {
    ...
    injectVariablesWithSpecialProperties()
  ...
  function injectVariablesWithSpecialProperties () {
    ...
    VARIABLES._runChain = (chain, ...args) => {
      ...
      let sentVariables = {$: dollar}
      executeChain(chain, args, sentVariables, callback)
    }
    VARIABLES._map = (list, chain, callback) => {
      map(list, chain, getStrippedVariables(), callback)
    }
    VARIABLES._filter = (list, chain, callback) => {
...
function map (list, chain, variables, callback) {
  ...
  stringify = requireOnce('json-stringify-safe')
  let actions = []
  let chainScript = stringify(chain)
  for (let element of list) {
    actions.push((next) => {
      executeChain(chainScript, [element], variables, (error, result) => {
        next(error, result)
      })
    })
  }
