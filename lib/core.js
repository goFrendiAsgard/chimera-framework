...
function executeChain (chain, ins = [], vars = {}, callback = null) {
  ...
  fs.readFile(chain, (error, chainScript) => {
    ...
    return executeAndCompile(chainScript)
  })

  function executeAndCompile (chainScript) {
    // file exists
    let chainPath = path.resolve(chain)
    let compiledChainPath = chainPath + '.cjson'
    ...
    async.series([
      ...
      // get compiledChain fileStat
      (next) => {
        fs.stat(compiledChainPath, function (error, compiledChainStat) {
          if (error || chainStat.mtime > compiledChainStat.mtime) {
