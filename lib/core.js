...
function executeChain (chain, ins, vars, callback) {
  ...
  function evaluateStatement (statement, log = false) {
    ...
    if (util.isString(statement)) {
      ...
      statement.substr(0, 5) === 'file:' || statement.substr(0, 7) === 'script:') {
        return statement
      }
  ...
  function getNormalizedValue (val) {
    if (!util.isString(val) || NOT_JSON_CACHE.indexOf(val) > -1) {
      return val
    }
    try {
      return JSON.parse(val)
    } catch (error) {
      NOT_JSON_CACHE.push(val)
      return val
    }
  }
  ...
  function map (list, chain, variables, callback) {
    ...
    executeChain(chain, [list[0]], variables, (error, result) => {
      if (error) {
        return callback(error, null)
      }
      ...
      return nsync.map(list, action, callback)
    })
  }
