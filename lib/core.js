...
function executeChain (chain, ins, vars, callback) {
  ...
  function evaluateObjectStatement (statement) {
    ...
    Object.entries(statement).forEach(([key, val]) => {
      result[key] = evaluateStatement(val)
    })
    return result
  }
  ...
  function evaluateStatement (statement, log = false) {
    ...
    if (util.isString(statement)) {
      ...
      if (UNEVALUATED_STATEMENT.indexOf(statement) > -1 || statement === '' ||
        statement[0] === '/' || statement.substr(0, 5) === 'file:' || statement.substr(0, 7) === 'script:') {
        return statement
      }
      const fourFirstChar = statement.substr(0, 4)
      if (fourFirstChar === 'C:\\' || fourFirstChar === 'D:\\' || fourFirstChar === 'E:\\' ||
        fourFirstChar === 'F:\\' || fourFirstChar === 'G:\\' || fourFirstChar === 'H:\\') {
        return statement
      }
      if (statement.match(VALID_URL)) {
        // url
        return statement
      }
      try {
        ...
        if (statement.match(MERE_UNQUOTED_STRING_PATTERN)) {
        ...
        try {
          ...
          if (log && VERBOSE < VARIABLES._verbose) {
            logMessage(error)
          }
          UNEVALUATED_STATEMENT.push(statement)
          return statement
        }
      }
    }
  ...
  function map (list, chain, variables, callback) {
    ...
    nsync.map(list, action, callback)
  }
  ...
  function filter (list, chain, variables, callback) {
    ...
    nsync.filter(list, action, callback)
  }
