...
function execute(chainConfigs, argv, presets, finalCallback, chainOptions){
    ...
    function main(){
        ...
        if(chainConfigs === false){
            ...
            finalCallback(new Error('Unable to fetch chain', false)
            return null
        }
    ...
    function finalCallback(error, result){
        ...
        if(getVar('_error')){
            ...
            if(errorMessage == ''){
                errorMessage = 'Cannot execute chain'
            }
    ...
    function getSingleModuleChainRunner(chainCommand, chainOut, parameters, callback){
        ...
        try{
            processModule(moduleName, parameters, chainOptions.cwd, function(error, output){
    ...
    function runChains(chains, mode, isCoreProcess, runCallback){
        ...
        // run actions
        if(isCoreProcess){
            asyncRunner(actions, finalCallback)
        }
        else{
