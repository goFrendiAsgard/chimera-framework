...
function executeChain (chain, ins, vars, callback) {
    ...
    fs.readFile(chainPath, (error, chainScript) => {
      ...
      return chimlParser.parseChiml(chainScript, (error, chainObj) => {
        ...
        nsync.parallel([
          (next) => {
            // compile
            util.writeJsonFile(compiledChainPath, chainObj, next)
          },
          (next) => {
            // execute
            runRootChain(chainObj, util.getDeepCopiedObject(ins), vars, (error, result) => {
              executionResult = result
              next(error)
            })
          }
        ], (error, result) => {
          callback(error, executionResult)
        })
      })
    })
  }
  ...
  function createChainRunner (chain, finalCallback, firstTime = true) {
    ...
    let asyncWorker = isHavingChildren && chain.mode === 'parallel' ? nsync.parallel : nsync.series
    return (callback) => {
      let actions = []
      if (VARIABLES._error) {
