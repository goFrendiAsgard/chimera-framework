  ...
  return (request, response, next) => {
    ...
    function createChainProcessor (config) {
      return function (callback) {
        ...
        let urlPath = url.parse(STATE.request.url).pathname
        let method = STATE.request.method.toLowerCase()
        // delete STATE.config.vars && STATE.config.routes
        delete STATE.config.vars
        delete STATE.config.routes
        logMessage('BEFORE ' + urlPath, VERBOSE)
        ...
        for (let i = 0; i < routeObjList.length; i++) {
          ...
          if ((routeMethod === 'all' || routeMethod === method) && isRouteMatch(route, urlPath)) {
            ...
            logMessage('FIND ' + urlPath + ' -> ' + chain, VERBOSE)
            return core.executeChain(chain, [STATE], config.vars, function (error, newResponse) {
              ...
              logMessage('AFTER ' + urlPath + ' -> ' + chain, VERBOSE)
        ...
        logMessage('AFTER ' + urlPath + ' (Not Found)', VERBOSE)
        logState(urlPath + ' (Not Found)', VERY_VERBOSE)
        return callback()
      }
    }
...
function createSafeResponder (app, webConfig) {
  return (request, response, next) => {
    try {
      let responder = createResponder(app, webConfig)
      responder(request, response, next)
    } catch (error) {
      next()
    }
  }
...
function createApp (webConfig, ...middlewares) {
  ...
  let safeResponder = createSafeResponder(app, webConfig)
  app.all('/*', safeResponder)
