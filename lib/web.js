...
function createHookProcessor(webConfig, hookKey, hookErrorMessage){
    return function(callback){
        ...
        if(util.isRealObject(hook)){
            ...
            // hook is string, turn it into a wrapper function, and continue
            if(util.isString(hook)){
                let chain = hook
                hook = function(state, next){
                    core.executeChain(chain, [state], next)
                }
            }
            // hook is function
            if(util.isFunction(hook)){
                // hook is function. Execute it and run the callback
                hook(STATE, function(error, newState){
                    if(!error){
                        STATE = util.patchObject(STATE, newState)
                    }
                    else{
