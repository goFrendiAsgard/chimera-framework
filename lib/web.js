...
function createHookProcessor (webConfig, hookKey, hookCaption, hookErrorMessage) {
...
function createApp (webConfig) {
  ...
  console.error('CONFIG : ' + JSON.stringify(webConfig) + '\n')
  ...
  app.all('/*', (request, response, next) => {
    ...
    async.series([
      // startupRequest
      createHookProcessor(webConfig, 'startupHook', 'STARTUP HOOK', 'Unable to process startup hook'),
      // beforeRequest
      createHookProcessor(webConfig, 'beforeRequestHook', 'BEFORE REQUEST HOOK', 'Unable to process beforeRequest hook'),
      // process chain
      createChainProcessor(webConfig),
      // afterRequest
      createHookProcessor(webConfig, 'afterRequestHook', 'AFTER REQUEST HOOK', 'Unable to process afterRequest hook')
    ], (error, result) => {
