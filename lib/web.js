...
function getAppWithMiddleware (app, config) {
  ...
  app.use(fileUpload())
  // use view engines
  for (let extension in config.viewEngines) {
    let engine = engines[config.viewEngines[extension]]
  ...
  return (request, response) => {
    ...
    function finalProcess (app, request, response) {
      ...
      if ('server' in app && 'webSocket' in app.server && '_emit' in STATE.response) {
        try {
          let io = app.server.webSocket
          let emit = STATE.response._emit
          let rooms = 'rooms' in emit && util.isArray(emit.rooms) ? emit.rooms : []
          let event = 'event' in emit && util.isString(emit.event) ? emit.event : ''
          let args = 'args' in emit && util.isArray(emit.args) ? emit.args : []
          // prepare sockets
          let sockets = io.sockets
          for (let room of rooms) {
            sockets = sockets.to(room)
          }
