...
function getPackedResponse (res) {
  let packedRes = {}
  for (let key in DEFAULT_RESPONSE) {
    if (key in res) {
      packedRes[key] = res[key]
    } else {
      packedRes[key] = DEFAULT_RESPONSE[key]
    }
  }
  return packedRes
}
...
function getWebState (config, request, response) {
  ...
  return {
    ...
    'response': getPackedResponse(response)
  }
}
    ...
    function createHookProcessor (config, hookKey) {
      return function (callback) {
        ...
        if (util.isRealObject(hook)) {
          ...
          let chain = hook
          logState('BEFORE ' + hookKey, VERY_VERBOSE)
          return core.executeChain(chain, [STATE], {}, function (error, newState) {
            if (!error) {
              STATE = util.getPatchedObject(STATE, newState)
            } else {
      ...
      return function (callback) {
        ...
        for (let i = 0; i < routeObjList.length; i++) {
          ...
          if ((routeMethod === 'all' || routeMethod === method) && isRouteMatch(route, urlPath)) {
            ...
            return core.executeChain(chain, [STATE], {}, function (error, newResponse) {
              if (!error) {
