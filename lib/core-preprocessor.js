...
const KEY_ORDER = ['id', 'if', 'vars', 'ins', 'out', 'verbose', 'command', 'compiledCommand', 'mode', 'chains', 'while']
const KEY_SYNONYM = {
  'process': 'command',
  'do': 'command',
  'input': 'ins',
...
function getNonErrorPart (chain) {
  return util.getFilteredObject(chain, ['error', 'errorMessage'])
}
...
function getWrappedShellCommand (command) {
  return (...args) => {
    ...
    for (let value of args) {
      command += ' ' + util.quote(String(value).trim())
    }
...
function getChainWithStandardKey (chain) {
  let standardChain = {}
  // copy the alread standard properties
