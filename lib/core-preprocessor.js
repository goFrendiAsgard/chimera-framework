...
function getTrueRootChain (chain, isStandard = false) {
  util = requireOnce('./util.js')
  let CHAIN_ID = 1

  let ins, out, vars
  CHAIN_ID = 1
  chain = getTrueChain(chain, isStandard)
  ...
  function getWrappedShellCommand (command) {
    return (...args) => {
      ...
      for (let value of args) {
        ...
        shellCommand += ' ' + util.getQuoted(strValue.trim())
      }
  ...
  function getCompiledCommand (chain) {
    ...
    let singleLineCommand = chain.command.trim().replace(/\r|\n/g, '')
    let isNormalJs = singleLineCommand.match(MATCH_INSIDE_BRACES_PATTERN)
    let isCallbackJs = singleLineCommand.match(MATCH_INSIDE_SQUARE_BRACKETS_PATTERN)
    let isPromiseJs = singleLineCommand.match(MATCH_INSIDE_CHEVRONS_PATTERN)
    let isArrowFunction = singleLineCommand.match(MATCH_ARROW_FUNCTION_PATTERN)
  ...
  function getChainWithTrueCommand (chain) {
    if (util.isString(chain.command)) {
      let chainObj = splitCommandString(chain.command)
  ...
  function getTransformedMapAndFilter (chain) {
    ...
    let out = chain.into
    let subChain = {}
    let forbiddenKey = ['filter', 'map', 'into']
    for (let key in chain) {
    ...
    let newChain = {ins, out, command}
    return newChain
  }

  function getTrueChain (chain, isStandard = false) {
    if (isStandard) {
      chain = getCompiledCommand(chain)
      if ('chains' in chain) {
        for (let i = 0; i < chain.chains.length; i++) {
          chain.chains[i] = getTrueChain(chain.chains[i], true)
        }
      }
      return chain
    }
    ...
    if ('chains' in chain) {
      for (let i = 0; i < chain.chains.length; i++) {
        chain.chains[i] = getTrueChain(chain.chains[i], false)
      }
    }
    return chain
  }
