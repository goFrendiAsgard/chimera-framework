...
function execute (configs, fn, ...args) {
  let mongoUrl = 'mongoUrl' in configs ? configs.mongoUrl : ''
  let dbOption = 'dbOption' in configs ? configs.dbOption : {}
  let collectionName = 'collectionName' in configs ? configs.collectionName : ''
  delete configs.mongoUrl
  delete configs.dbOption
  delete configs.collectionName
  let manager = getManager(mongoUrl, dbOption)
  let collection = getCollection(manager, collectionName, configs)
  // wrap last argument
