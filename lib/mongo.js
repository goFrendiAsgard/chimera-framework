...
if (require.main === module) {
  ...
  async.series([
    ...
    (next) => {
      softGod.insert({name: 'Odin', mythology: 'Nordic', power: 6000}).then(
      (result) => {
    ...
    (next) => {
      softGod.insert([{name: 'Tyr', mythology: 'Nordic', power: 4000}, {name: 'Posseidon', mythology: 'Greek', power: 3000}, {name: 'Zeus', mythology: 'Greek', power: 7000}]).then(
      (result) => {
    ...
    (next) => {
      softGod.update({name: 'Tyr'}, {name: 'Thor'}).then(
      (result) => {
    ...
    (next) => {
      softGod.softRemove({name: 'Posseidon'}).then(
      (result) => {
    ...
    (next) => {
      softGod.aggregate([{'$group': {'_id': 'count', 'count': {'$sum': 1}}}]).then(
      (result) => {
