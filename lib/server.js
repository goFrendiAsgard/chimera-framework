...
function serve (callback) {
  ...
  async.parallel([
    (next) => {
      fs.access(path.join(processCwd, 'startup.yaml'), fs.constants.R_OK, (error) => {
        if (!error) {
    ...
    (next) => {
      fs.access(path.join(processCwd, 'beforeRequest.yaml'), fs.constants.R_OK, (error) => {
        if (!error) {
    ...
    (next) => {
      fs.access(path.join(processCwd, 'afterRequest.yaml'), fs.constants.R_OK, (error) => {
        if (!error) {
