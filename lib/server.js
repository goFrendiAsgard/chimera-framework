    ...
    async.series([
        (next) => {
            fs.access(process.cwd() + '/startup.yaml', fs.constants.R_OK, function(error){
                if(!error){
                    webConfig['startupHook'] = process.cwd() + '/startup.yaml'
                    console.warn('Startup hook found')
                }
                else{
                    console.warn('No startup hook defined')
                }
                next()
            })
        },
        (next) => {
            fs.access(process.cwd() + '/beforeRequest.yaml', fs.constants.R_OK, function(error){
                if(!error){
                    webConfig['beforeRequestHook'] = process.cwd() + '/beforeRequest.yaml'
                    console.warn('BeforeRequest hook found')
                }
                else{
                    console.warn('No beforeRequest hook defined')
                }
                next()
            })
        },
        (next) => {
            fs.access(process.cwd() + '/afterRequest.yaml', fs.constants.R_OK, function(error){
                if(!error){
                    webConfig['afterRequestHook'] = process.cwd() + '/afterRequest.yaml'
                    console.warn('AfterRequest hook found')
                }
                else{
                    console.warn('No afterRequest hook defined')
                }
                next()
            })
        }
