...
function runCallbackAndReturn(callback, output, error=null){
...
function applyDefaultCallback(callback){
...
function eisn(srcFile, dstFile, command, finalCallback){
    finalCallback = applyDefaultCallback(finalCallback)
    // define result
...
function parseJsonOrYaml(str, callback){
    ...
    return runCallbackAndReturn(callback, obj, error)
}
...
function readJsonOrYaml(file, callback){
    fs.readFile(file, function(error, fileContent){
        if(error){
            ...
            parseJsonOrYaml(fileContent, callback)
        }
    })
}
...
module.exports = {
    ...
    'applyDefaultCallback' : applyDefaultCallback,
}
