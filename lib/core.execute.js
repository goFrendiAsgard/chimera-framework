    ...
    try{
        if(util.isNullOrUndefined(value)){
            ...
            if(util.isString(value)){
                try{
                    log('Getting parsed value '+stringify(value))
                    val = eval(value)
                    value = val
                }
                catch(error){}
            }
            log('Assign value {'+typeof(value)+'} '+stringify(value)+' to variable '+key)
            eval('VARS.'+key+' = value')
        }
    }
    catch(error){
        log('Cannot assign value {'+typeof(value)+'} '+stringify(value)+' to variable '+key)
        console.error(error)
        VARS._error = true
...
function getVar(key, verbosity=1){
    let result
...
function createChainRunner(chain, finalCallback){
    log('Create chain runner for: ' + chain.id)
    ...
    return (callback) => {
        ...
        if(checkCondition(chain.if)){
            // if chainRunner
            log('If condition resolved for '+chain.id+': ' + chain.if)
            if('chains' in chain && util.isArray(chain.chains)){
                ...
                actions.push((next)=>{
                    ...
                    try{
                        log('Run ' + JSON.stringify(chain.original_command) + ' with arguments: '+ JSON.stringify(inputs))
            ...
            // else chainRunner
            log('If condition rejected for '+chain.id+': ' + chain.if)
            actions.push(createChainRunner(chain.else, finalCallback))
        }
        log('Finish chain: '+chain.id)
        // error checker
        if(getVar('_error')){
            ...
            log('Error condition rejected for: ' + chain.id)
        }
        // while chainRunner 
        actions.push((next)=>{
...
function initVars(chain, ins, vars){
    ...
    // set vars
    log('Preset variables: ' + JSON.stringify(vars))
    for(key in vars){
        setVar(key, vars[key])
    }
    // set ins
    log('Inputs: ' + JSON.stringify(ins))
