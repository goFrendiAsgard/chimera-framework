...
async.series([
    // test control-max
  (next) => {
    testChain('Test control max',
        'tests/control-max.yaml', [5, 5], next)
  },
    // test sprout
  (next) => {
    testChain('Test Sprout',
        'tests/sprout.yaml', [4, 5], 18, next)
  },
    // test stretch string
  (next) => {
    testChain('Test Stretch-string',
  ...
  (next) => {
    ...
    // test execute chain
  (next) => {
    testChain('Test executeChain without presets',
        'tests/minimal.yaml', [1, 5], -23, next)
  },
  (next) => {
    testChain('Test executeChain with presets',
        'tests/minimal.yaml', [1, 5], {'a': 1, 'b': 1}, -23, next)
  },
  (next) => {
    testChain('Test executeChain containing empty object',
        'tests/empty.yaml', [0], '', next)
  },
  (next) => {
    testChain('Test executeChain containing infinite loop, expect error',
        'tests/infinite-loop.yaml', [0], '', next)
  },
    // test execute command
  (next) => {
    testCmd('Test error handling: no error',
        'chimera tests/error-handling.yaml 6 6', 12, next)
  },
  (next) => {
    testCmd('Test error handling: error less',
        'chimera tests/error-handling.yaml 5 6', '', next)
  },
  (next) => {
    testCmd('Test error handling: error more',
        'chimera tests/error-handling.yaml 6 5', '', next)
  },
  (next) => {
    testCmd('Test Empty process with single argument',
        'chimera "(a)->-> b" 6', 6, next)
  },
  (next) => {
    testCmd('Test Empty process with two argument',
        'chimera "(a,b)->->(c)" 6 5', '[6,5]', next)
  },
  (next) => {
    testCmd('Test Empty process with single argument and shorthand',
        'chimera "(a)--> b" 6', 6, next)
  },
  (next) => {
    testCmd('Test Empty process with two argument and shorthand',
        'chimera "(a,b)-->(c)" 6 5', '[6,5]', next)
  },
  (next) => {
    testCmd('Test JSON instead of YAML',
        'chimera tests/add.json 1 5', 6, next)
  },
  (next) => {
    testCmd('Test javascript arrow function',
        'chimera tests/add-js.yaml 1 5', 6, next)
  },
  (next) => {
    testCmd('Test complete',
        'chimera tests/complete.yaml 1 5', -23, next)
  },
  (next) => {
    testCmd('Test minimal',
        'chimera tests/minimal.yaml 1 5', -23, next)
  },
  (next) => {
    testCmd('Test inline-1',
        'chimera "(a, b) -> node tests/programs/add.js -> c" 1 5', 6, next)
  },
  (next) => {
    testCmd('Test inline-2',
        'chimera "(a, b) -> node tests/programs/add.js" 1 5', 6, next)
  },
  (next) => {
    testCmd('Test implode',
        'chimera tests/implode.yaml 1 2 3', '1, 2, 3', next)
  },
  (next) => {
    testCmd('Test control-1',
        'chimera tests/control.yaml 5', 8, next)
  },
  (next) => {
    testCmd('Test control-2',
        'chimera tests/control.yaml 12', 11, next)
  },
  (next) => {
    testCmd('Test simple-command',
        'chimera tests/simple-command.yaml 5 6', 11, next)
  },
  (next) => {
    testCmd('Test nested-control',
        'chimera tests/nested-control.yaml', '1112*1314**2122*2324**3132*3334**', next)
  },
  (next) => {
    testCmd('Test complex-vars',
